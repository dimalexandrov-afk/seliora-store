export type StoreImage={id:string;storage_path:string;alt_text:string|null;sort_order:number;is_primary:boolean};
...
export async function getProducts():Promise<StoreProduct[]>{try{return await supabaseFetch('products?select=id,slug,name,subtitle,description,denier,finish,base_price_eur,launch_price_eur,sort_order,product_images(id,storage_path,alt_text,sort_order,is_primary),product_variants(id,sku,color_name,color_hex,size_label,active)&status=eq.active&order=sort_order.asc&product_images.order=sort_order.asc')}catch{return []}}
export async function getProduct(slug:string):Promise<StoreProduct|null>{try{const rows=await supabaseFetch('products?select=id,slug,name,subtitle,description,denier,finish,base_price_eur,launch_price_eur,sort_order,product_images(id,storage_path,alt_text,sort_order,is_primary),product_variants(id,sku,color_name,color_hex,size_label,active)&status=eq.active&slug=eq.'+encodeURIComponent(slug)+'&limit=1&product_images.order=sort_order.asc');return rows[0]??null}catch{return null}}
