...
export async function getProduct(slug:string):Promise<StoreProduct|null>{
  try{
    const rows=await supabaseFetch('products?select='+selectFields+'&status=eq.active&slug=eq.'+encodeURIComponent(slug)+'&limit=1&product_images.order=sort_order.asc');
    return rows[0]??null;
  }catch{return null}
}
...
export function primaryImage(p:StoreProduct){
  const imgs=[...(p.product_images??[])].sort((a,b)=>(b.is_primary?1:0)-(a.is_primary?1:0)||a.sort_order-b.sort_order);
  return imgs[0]??null
}
