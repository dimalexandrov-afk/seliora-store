...
type Variant={id:string;product_id:string;sku:string;color_name:string;color_hex:string|null;size_label:string;active:boolean};
type Stock={variant_id:string;quantity_on_hand:number;quantity_reserved:number;reorder_level:number};
type ProductImage={id:string;product_id:string;storage_path:string;alt_text:string|null;sort_order:number;is_primary:boolean;color_name:string|null};
...
export default function AdminPage(){
  ...
  async function selectProduct(productId:string){
    ...
    const[{data:v,error},{data:imgs}]=await Promise.all([
      supabase.from('product_variants').select('id,product_id,sku,color_name,color_hex,size_label,active').eq('product_id',productId).order('size_label'),
